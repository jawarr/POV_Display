...
int main(void)
{
    ...
    for (i = 0; i < data.fieldCount; i++)
    {
		...
		if (isCommand(&data, "word", 1))
		{
			char *text = getFieldString(&data, 1);
			putsUart("You have typed in the word: ");
			putsUart(text);
			putcUart('\n');
		...
		else if (isCommand(&data, "number", 1))
		{
			int value = getFieldInteger(&data, 1);
			
			putsUart("You typed a number: ");
			char buffer[12];
